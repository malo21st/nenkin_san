...
st.sidebar.image(get_pdf_image(docu_type, 0), caption = docu_type, use_column_width = "auto")
...
if st.session_state.qa["history"][-1]["role"] == "Q":
    ...
    try:
        ...
        # refer_pages = f"\n\n参照：{page_int} ページ\n\n\n"
        page_lst = [int(node.extra_info['page_label']) - 1 for node in response.source_nodes]
...
#             error_msg = "エラーが発生しました！　もう一度、質問して下さい。"
        ...
        st.session_state.qa["history"].append({"role": "E", "msg": f"ERROR: {error_msg}"})
